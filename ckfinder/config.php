...
//$baseUrl = "/var/www/XLabs/ckedit/ckfinder/userfiles/"
//exit(dirname(dirname(dirname(dirname(dirname(__FILE__))))));
$installDir=dirname(dirname(dirname(dirname(dirname(__FILE__)))))."/";
if(isset($_SESSION["LGKS_EDITOR_FPATH"])) {
	$px=$_SESSION["LGKS_EDITOR_FPATH"];
	$baseDir = $installDir.$px;
	$baseUrl = $px;
	
	if(strpos($px,"apps")===0) {
		$urlArr=explode("/",$baseUrl);
		$cacheURL=$installDir . "apps/{$urlArr[1]}/tmp/cache/_thumbs";
		$cacheDir=$installDir . "apps/{$urlArr[1]}/tmp/cache/_thumbs";
	} else {
		$cacheURL=$installDir . '/tmp/cache/_thumbs';
		$cacheDir=$installDir . '/tmp/cache/_thumbs';
	}
} else {
	$baseDir = $installDir."media/";
	$baseUrl = "media/";
...
//exit($baseDir);
//if(count($urlArr)>2) $baseUrl="{$urlArr[0]}/{$urlArr[1]}/tmp/";
//exit($baseUrl);//dirname(dirname(dirname(dirname(dirname(__FILE__))))). '/tmp/cache/_thumbs'
//exit(dirname($baseUrl) . '/tmp/_thumbs');
...
*/
$config['HideFolders'] = Array(".svn",".git", "CVS");
